http://rpubs.com/klarnolds/275744
---
...
```{r}
cleaned_data %>% filter(time.point ==  "6hpi") %>% arrange(desc(value)) %>% slice(1) %>% select(plant)
```
...
```{r}
cleaned_data %>% filter(time.point == "18hpi") %>% arrange(desc(value)) %>% slice(1) %>% select(plant, fungus)
```
...
mutate(time.value = str_replace(time.point, 'hpi', ''),
       time.hpi = str_c('hpi.', time.value)) -> cleaned_data
cleaned_data
```
...
summarise(expr.var = var(expr.diff), abs_mean = mean(abs(expr.diff)))-> cleaned_data_expr
```
...
```{r}
cleaned_data_expr%>%
arrange(desc(expr.var)) %>% group_by(plant, fungus) %>% select(gene.name, plant, fungus) %>% slice(3) -> cleaned_data_top3
cleaned_data_top3
```
...
The above `cleaned_data` allows us to compare mRNA expression `value` of fungal genes when looking at the following `plant_fungus_combos`. Examining the `expr.var` allows to see which genes may be of interest in this `df`. The genes with the greatest variable expression, `cleaned_data_top3`' would be of higest interest, notably `%filter(gene.name == 'bgh06162'` shows up twice in this list making it a potentially critical gene.

## Problem 5
```{r}
cleaned_data_expr%>%
arrange(desc(expr.var)) %>% group_by(plant, fungus) %>% slice(3) -> cleaned_data_top3
cleaned_data_top3

merge(cleaned_data_top3, cleaned_data, by = c ("gene.name", "plant", "fungus")) -> merge_data
merge_data
```
...
```{r}
ggplot(merge_data, aes(x = as.numeric(time.value), y= value, color =gene.name)) + geom_point() +geom_smooth(method = lm, se =FALSE) + facet_grid(plant ~ fungus) + theme_bw()-> data_plot
data_plot + labs(x= "Hours Post Infection", y = "mRNA Expression", title = "Highest Differential Gene Expression")
```
